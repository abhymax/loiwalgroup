...
<section class="content">
				...
				<div class="box col-lg-6 right-panel-big" style="width:100% !important;">
                    <div class="card innerCards">
                        <div>
							...
							<div class="body">
                                <form id="addMaterialout"  class="form-horizontal" enctype="multipart/form-data" autocomplete="off">
                                    <div class="">
										<div class="row clearfix">
										...
										<div class="row clearfix">
										   <div class="col-lg-2 col-md-2 col-sm-4 col-xs-5 form-control-label">
											...
											<div class="col-lg-2 col-md-2 col-sm-4 col-xs-5 form-control-label">
												<label for="district">District<span class="col-pink">*</span></label>
											</div>
											<div class="col-lg-4 col-md-10 col-sm-8 col-xs-7">
												<div class="form-group">
													<div class="form-line">
                                                        ...
                                                        <button id="buttonDis" type="button">Add District</button>														
													</div>
												</div>
												<div id="dialogDistrict">
												...
												<table id="table_block" class="table-box dataTable">
													...
													<tbody>
														...
														<tr id="tr_block_{{ $loop->index }}">
															...
															<td>
															    <select name="product_id_{{$loop->index}}" id="product_id_{{$loop->index}}" class="product_name" onchange="getDetails({{$loop->index}})">
																<option value="">Select</option>
																</select>
															...
															<td>
															    <select name="batch_no_{{$loop->index}}" id="batch_no_{{$loop->index}}" onchange="getRate({{$loop->index}})">
																<option value="">Select</option>
																</select>
															</td>
															<td>
																<input type="number" name="qty_{{ $loop->index }}" value="{{$podinfo['qty']}}" onkeypress="return event.charCode >= 48" min="1" onchange="calculatePrice({{ $loop->index }})" id="qty_{{ $loop->index }}"  maxlength="20">
															</td>
															<td>
																<input type="number" name="rate_{{ $loop->index }}" value="{{$podinfo['rate']}}" maxlength="50" id="rate_{{ $loop->index }}" onchange="calculatePrice({{ $loop->index }})">
															</td>
															<td>
	...
	<div id="district_add" style="display:none">
		District:<br/>
		<input type="text" name="district_name" id="district_name">
		<br/>
		<input type="button" value="Save" onclick="saveDistrict()">
		<span class="font-bold col-pink" id="error-district"></span>
	</div>
</section>
...
<script type="text/javascript">
	...
	$('#buttonDis').click(function() {
	  $("div#district_add").dialog({
		appendTo: '#dialogDistrict',
		title: "Add District"
	  });
	});
	...
	function saveDistrict()
	{
		...
		{
		$('#district_name').css('border' , '1px solid red');
		} else {
			...
			$.ajax({
				type: 'POST',
				url: site_url+'/materialout/addDistrict',
				data: formData,
				contentType: false,
				processData: false,
				success: function(response){
					if(response.errorStatus){
						...
						$('#error-district').text(response.errormessage);
						
					} else {
						$('#district_name').css('border' ,'');
						$('.district').html(response);
						console.log(response);
						$( "div#district_add" ).dialog("close");
					}
				},
