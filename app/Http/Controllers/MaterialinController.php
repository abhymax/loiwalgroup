...
class MaterialinController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $query = Invoicein::with('supplier')->orderBy('date_added');
        ...
        if($request->ajax()){
            return view('invoice/invoice_list_ajax', compact('data'));
        }
        else{
            return view('invoice/invoice_list', compact('data'));
        }
    }
	public function add()
	{
        ...
        $podinfo[0]['batch_no'] = '';
        $podinfo[0]['description'] = '';
		$podinfo[0]['qty'] = '';
        $podinfo[0]['rate'] = '';
        $podinfo[0]['amount'] = '';
        $podinfo[0]['manufacturing_date'] = '';
		$podinfo[0]['expiry_date'] = '';
        $podinfo[0]['material_id'] = '';
        $data['podinfo'] = $podinfo;
		$data['block_count'] = 1;
		
		return view('invoice/invoice_add', compact('data'));
	}
    
	public function getproduct($id){
		$products = Product::where('supplier_id', $id)->orderBy('product_name')->get();
		$phtml='<option value="">Select</option>';
        foreach($products as $product)
		{
		   $phtml.='<option value="'.$product->product_id.'" prodsku="'.$product->product_sku.'">'.$product->product_name.'</option>';
		}
		return $phtml;
    ...
    public function store(Request $request){
        ...
        $errorMessages = [
                ...
                'receive_date.required' => 'Receive date required',
				'transport_document_number' => 'Transport document number required'
            ];
        $validation = Validator::make($dataValidation, [
                'invoice_number' => 'required|unique:invoice_in,invoice_number',
                'supplier_id' => 'required',
				'invoice_date' => 'required',
                'receive_date' => 'required',
				'transport_document_number' => 'required'
            ], $errorMessages);
            if ($validation->fails()) {
                ...
                $invinfo = Invoicein::create($inputs);
				if($request['block_count']!=0)
				{
					for($i=0;$i<$request['block_count'];$i++)
					{
						if($request['product_id_'.$i]!='' && $request['qty_'.$i]!=''){
							...
							$podinfo['description'] = $request['description_'.$i];
							$podinfo['qty'] = $request['qty_'.$i];
							$podinfo['rate'] = $request['rate_'.$i];
							$podinfo['amount'] = $request['amount_'.$i];
							$podinfo['manufacturing_date'] = $request['manufacturing_date_'.$i];
							$podinfo['expiry_date'] = $request['expiry_date_'.$i];
							Materialin::create($podinfo);
							...
							$logentry['log_type'] = 'I';
							$logentry['log_details'] = 'Material In';
							$logentry['qty'] = $request['qty_'.$i];
							$logentry['entry_date'] = $inputs['receive_date'];
							...
							if($available)
							{
								...
								$entry['supplier_id'] = $inputs['supplier_id'];
								$entry['product_id'] = $request['product_id_'.$i];
								$entry['batch_no'] = $request['batch_no_'.$i];
								$entry['available_qty'] = $request['qty_'.$i];
								$entry['product_sku'] = $request['product_sku_'.$i];
								Stockavailable::create($entry);
							}
						}
					}
				}
	...
	public function view($id){
        ...
        // get the building data
        $invoiceinfo = Invoicein::with('supplier')->where('id',$id)
                        ->first();
        
        $materialinfo = Materialin::with('product')->where('invoice_id',$id)->get();
		...
		return view('invoice/invoice_view', compact('data'));
