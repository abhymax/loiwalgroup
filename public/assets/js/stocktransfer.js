$(function () {
    ...
    // Building update end
    $('#searchtransfer').validate({
        
        submitHandler: function(form,e) {
            searchData('stocktransfer', 'searchtransfer', 'transferListTableDiv', 'transferListTable'); //module, formid, divToBeReplaced, tableId
        }
    });
...
function addStockTransfer(){
    ...
    let formData = new FormData($('#addStocktransfer')[0]);
	$.ajaxSetup({
	  headers: {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            setTimeout(function(){
				...
				window.location.href = site_url+'/stocktransfer';
...
function  getDetails(id)
{
	...
	if(selected_product){
		$.ajax ({
			type: 'GET',
			url: site_url+'/stocktransfer/getstock/'+selected_product,
